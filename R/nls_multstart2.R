...
wfct2 <- function(expr)
{
  ...
  nlsLMfound <- FALSE
  generation <- -1
  # iterate the call stack from bottom to top
  #message(paste0(length(sys.calls())," calls on stack!"))
  for (i in length(sys.calls()):1) {
    generation <- generation+1
    #message(paste0("Examining call stack ",i," of generation ",generation))
    tmpList <- as.list(sys.calls()[[i]])
    if(!is.null(tmpList[["data"]])) {
      mc <- sys.calls()[[i]]
      mcL <- tmpList
      for(j in 1:length(mcL)) {
        if(any(grepl("nlsLM", mcL[[j]]))) {
            #message(paste("Found call stack with data", paste(mcL, collapse = ","), "\n", sep=" "))
            #message(paste("Found call stack with names", paste(names(mcL), collapse = ","), "\n", sep=" "))
            nlsLMfound <- TRUE
            break
        }
      }
  ...
  #mc <- sys.calls()[[1]]
  #mcL <- as.list(mc)
  #message(paste("Using call stack with data", paste(mcL, collapse = ","), "from", generation, "generations before", "\n", sep=" "))
  ## get data and write to newEnv
  DATA <- mcL[["data"]]
  #message(paste(DATA, "\n", collapse=","))
  DATA <- eval(DATA, envir=parent.frame(n = generation))
  DATA <- as.list(DATA)
  #message(paste(DATA, "\n", collapse=","))
  ...
  for (i in 1:length(DATA)) {
    #message(paste0("For name ",NAMES[i], ": adding data ",DATA[[i]], "\n"))
    assign(NAMES[i], DATA[[i]], envir = newEnv)
  }
  #message("Assigned NAMES and DATA")
  ## get parameter, response and predictor names
  #message(paste0("Formula: ", mcL[[2]]))

  mcL[[2]]
  if(is.expression(mcL[[2]])) {
    formula <- as.formula(mcL[[2]])
  } else {
    formula <- eval(mcL[[2]], envir = parent.frame(n = generation))
  }
  #message(paste0("Assigned formula as: ", as.expression(formula)))
  VARS <- all.vars(formula)
  #message("Assigned VARS")
  RESP <- VARS[1]
  #message("Assigned RESP")
  RHS <- VARS[-1]
  #message("Assigned RHS")
  PRED <- match(RHS, names(DATA))
  #message("Assigned PRED")
  PRED <- names(DATA)[na.omit(PRED)]
  #message("Assigned PRED2")

  #message("Extracted formula, variables etc")
  ## calculate variances for response values if "error" is in expression
  ## and write to newEnv
  if (length(grep("error", expr)) > 0) {
    #message("Evaluating error expression")
    y <- DATA[[RESP]]
    x <- DATA[[PRED]]
  ...
  if (length(grep("fitted", expr)) > 0 || length(grep("resid", expr)) > 0) {
    #message("Evaluating fitted / resid")
    mc2 <- mc
    mc2$weights <- NULL
    MODEL <- eval(mc2, envir = parent.frame(n = generation))
    fitted <- fitted(MODEL)
...
function(formula, data = parent.frame(), iter, start_lower, start_upper,
         supp_errors = c("Y", "N"), convergence_count = 100, control, modelweights, ...) {
  ...
  if (missing(modelweights)) {
    nlsData$modelweights <- rep(1, length(nlsData[[params_dep]]))
    nlsData$modelweights
    #huberWeights <- flipr::huberLoss2(nlsData[[params_dep]], median(nlsData[[params_dep]]))
  } else {
    nlsData$modelweights <- eval(substitute(modelweights), nlsData)
  }
    ...
    for (j in 1:nrow(strt)) {
      ...
      try(
        fit <- minpack.lm::nlsLM(
          ...
          control = control,
          data = nlsData,
          #weights = flipr::wfct(1/(flipr::huberLoss(resid, 0.01))), ...
